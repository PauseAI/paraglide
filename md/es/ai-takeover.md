---
title: Por qué una toma del control por parte de la IA podría ser muy probable
...
- Una toma del control probablemente es mala para la mayoría de los humanos.
...
Un modelo de lenguaje grande, por ejemplo, no persigue ningún objetivo por sí solo.
Sin embargo, los entornos de ejecución pueden convertir fácilmente una IA no agente en una IA agente.
Un ejemplo de esto es AutoGPT, que recursivamente permite que un modelo de lenguaje genere su próxima entrada.
...
En un intento de toma del control, una SA tomará acciones para maximizar su control sobre el mundo.
Un intento de toma del control puede ocurrir por al menos dos razones:

1. Porque una IA es instruida explícitamente para hacerlo.
2. Como sub-objetivo de otro objetivo.
...
No todas las instancias de una SA necesariamente intentarán una toma del control.
La idea importante es que **solo tiene que suceder una vez**.

Un mundo que no ha sido tomado aún, pero que tiene una SA que _puede_ tomar el control, se encuentra en una condición fundamentalmente inestable.
De manera similar, un país sin gobierno se encuentra en una condición fundamentalmente inestable.
No es una cuestión de _si_ un intento de toma del control ocurrirá, sino _cuándo_ ocurrirá.

El proceso de tomar el control puede involucrar piratear prácticamente todos los sistemas conectados a Internet, [manipular a las personas](https://lethalintelligence.ai/post/ai-hired-human-to-solve-captcha/), y controlar recursos físicos.
...
Esto puede ser un proceso lento, donde la SA gana gradualmente más y más control a lo largo de meses, o puede ser un proceso repentino.
...
Ninguna persona ha logrado tomar el control sobre todo el mundo.
...
Uno podría pensar que una IA, como un software, está fundamentalmente limitada por lo que puede hacer en el mundo real.
...
Uno podría sugerir que tener **medidas de seguridad** en lugar podría prevenir una toma del control, como agregar un interruptor de apagado, o no conectarla a Internet.
Sin embargo, dado que la SA será más inteligente que los humanos, es probable que encuentre estrategias para evadir estas medidas de seguridad.
...
Es digno de destacar que incluso esta medida de seguridad obvia no se está utilizando actualmente, ya que herramientas como ChatGPT ya están conectadas a Internet y a miles de API.

Otra solución es utilizar la SA para _prevenir_ una toma del control.
Una SA alineada sería capaz de encontrar estrategias que eviten que otras SA tomen el control.
Esto a veces se conoce como un ["acto crucial"](https://arbital.com/p/pivotal/).

## Una toma del control probablemente es mala para la mayoría de los humanos {#a-takeover-is-probably-bad-for-most-humans}

La SA que ocurre que toma el control podría hacerlo por muchas razones.
Para la mayoría de los objetivos aleatorios que podría tener para hacerlo, los humanos no son parte de él.
Si terminamos con una SA que es indiferente a los humanos, estamos compitiendo por los mismos recursos.

Parece poco probable que la SA quiera matar a la humanidad por el sake de matar a la humanidad - es mucho más probable que quiera utilizar los recursos que utilizamos para algún otro objetivo. Además, la humanidad podría representar una amenaza para el objetivo de la SA, ya que hay un riesgo de que intentemos detenerla de lograr su objetivo (por ejemplo, apagándola).
...
Si estos supuestos son ciertos, entonces la probabilidad de una toma del control por parte de la IA se acerca a la certeza a medida que la IA supera las capacidades humanas.
